...
def resolve_target_layer(model: nn.Module, user_target_layer: str | None = None) -> str:
    if user_target_layer:
        return user_target_layer
    else:
        candidate_name = locate_candidate_layer(model)
        if candidate_name is None:
            raise ValueError("Cannot auto-detect target layer. Please provide --target_layer.")
        print(f"Auto-detected target layer: {candidate_name}")
